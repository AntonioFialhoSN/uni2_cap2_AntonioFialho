...
#include "funcoes_neopixel.h"  // Para as funções de aleatoriedade
...
// Adicionar esta função auxiliar para gerar cores aleatórias que não sejam verdes
rgb_color_t gerar_cor_aleatoria_sem_verde() {
    rgb_color_t cor;
    do {
        cor.r = numero_aleatorio(0, 65535);
        cor.g = numero_aleatorio(0, 65535);
        cor.b = numero_aleatorio(0, 65535);
        // Garante que o componente verde não seja dominante
    } while (cor.g > cor.r && cor.g > cor.b);  // Evita tons verdes
    return cor;
}
 ...
 */
// Modificar a função tratar_mensagem
void tratar_mensagem(MensagemWiFi msg) {
    ...
    if (msg.tentativa == 0x9999) {
        if (msg.status == 0) {
            ...
            // Gera cor aleatória que não seja verde
            rgb_color_t cor_aleatoria = gerar_cor_aleatoria_sem_verde();
            set_rgb_pwm(cor_aleatoria.r, cor_aleatoria.g, cor_aleatoria.b);
            
            // Mantém a cor por 1 segundo
            sleep_ms(1000);
            
            // Volta para a cor verde padrão
